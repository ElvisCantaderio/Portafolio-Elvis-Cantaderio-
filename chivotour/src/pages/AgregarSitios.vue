<template>
  ...
  <div class="text-center">
    <h5 class="titleSite">
      ...
      {{ $t('agregarSitio.agregarSitioTuristico') }}

      <!--Aqui se muestra la foto y el nombre de usuario para moviles-->

      <q-avatar size="30px" class="user-login lt-sm" style="position: absolute;">
        <img
      ...
      <q-form @submit.prevent="submitForm">
        ...
        <div class="row">
          ...
          <div class="q-pa-md col-md-6 col-sm-6 col-xs-6 text-left">
            <q-btn color="positive" :loading="cargando" icon="save" :label="$t('agregarSitio.enviar')" class="q-mt-md"
              @click="validarInputs" />
          </div>
...
<script>
import { Nsitios, Nimagenes } from "../boot/pouchdb.js";
...
export default {
  ...
  methods: {
    ...
    datosCreatedBy() {
      return { "usuario": this.getUsuario(), "email": this.getEmail(), "fecha": this.getFecha() };
    },
    datosMedia() {
    ...
    async submitForm() {
      ...
      this.UImagenes = [];
      try {
        var response = await Nsitios.post(data);
